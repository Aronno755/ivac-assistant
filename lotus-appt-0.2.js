/**
 * Lotus Appt 1.0 - Enhanced Automation Edition (FIXED)
 * Fixed manual submission retry issue
...
(function() {
    ...
    function createMainPanel() {
        ...
        container.innerHTML = `
            ...
            ">
                <div style="display: flex; align-items: center; gap: 10px;">
                    ...
                    <h3 style="margin: 0; font-size: 18px; font-weight: bold; text-shadow: 2px 2px 4px rgba(0,0,0,0.3);">Lotus Appt 1.0 (Fixed)</h3>
                </div>
        ...
        console.log('IVAC Assistant v6.0 (Fixed) loaded successfully');
    }
    ...
    function createBookingTabContent() {
        return `
            ...
            <!-- Manual Controls Row - ALWAYS ENABLED FOR TESTING -->
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin-bottom: 15px;">
    ...
    function createPaymentTabContent() {
        return `
            ...
            <!-- Captcha Token Field -->
            <div style="margin-bottom: 15px;">
                <label style="display: block; margin-bottom: 5px; font-weight: bold; color: #333; font-size: 13px;">🔐 Captcha Token</label>
    ...
    function setupBookingEventListeners() {
        ...
        // Manual controls - ALWAYS ENABLED FOR TESTING
        document.getElementById('submit-application-info').addEventListener('click', submitApplicationInfoManual);
        document.getElementById('submit-personal-info').addEventListener('click', submitPersonalInfoManual);
        document.getElementById('submit-overview').addEventListener('click', submitOverviewManual);
    }
    ...
    function setupPaymentEventListeners() {
        ...
        // Captcha
        document.getElementById('load-payment-captcha').addEventListener('click', loadPaymentCaptcha);
    ...
    // RETRY LOGIC - ONLY FOR AUTOMATION (Auto Login, Auto Booking)
    async function retryableRequest(url, options, statusCallback) {
        let retryCount = 0;
        const maxRetries = 10;
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                if (automationControl.login.stop || automationControl.booking.stop) {
                ...
                // FORBIDDEN - Stop immediately (no retry)
                if (result.status === 403 || result.status_code === 403) {
                    throw new Error('403 Forbidden - Automation stopped');
                }

                // NO SLOTS (422) - Stop immediately (no retry)
                if (result.status === 422 || result.status_code === 422) {
                    throw new Error('422 Unprocessable Entity - No slots available');
                }

                // RATE LIMIT - Retry with longer wait
                if (result.status === 429 || result.status_code === 429) {
                    retryCount++;
                    const waitTime = retryCount === 1 ? 30 : 60;
                    statusCallback(`Rate limit hit (429). Waiting ${waitTime} seconds... (Attempt ${retryCount}/${maxRetries})`, 'warning');
                    await sleep(waitTime * 1000);
                    continue;
                }

                // GATEWAY TIMEOUT - Retry quickly
                ...
                // SERVER ERROR - Retry with medium wait
                ...
                // If it's a known error that shouldn't retry, throw it
    ...
    async function startAutoLogin() {
        ...
        if (!mobileNumber || !password) {
            updateStatus('⚠️ Please fill in mobile number and password before starting auto login', 'warning');
            return;
        }
        ...
        startBtn.textContent = '⏳ Auto Login Running...';

        try {
            // Step 1: Load captcha
            updateStatus('🔄 Step 1/4: Loading captcha...', 'info');
            loadCloudflareWidget();

            // Wait for captcha token
            let captchaToken = '';
            for (let i = 0; i < 60; i++) {
                if (automationControl.login.stop) throw new Error('Stopped by user');
                
                captchaToken = document.getElementById('captcha-token').value.trim();
                if (captchaToken) break;
                
                updateStatus(`🔄 Step 1/4: Waiting for captcha verification... (${i+1}s)`, 'info');
                await sleep(1000);
            }

            if (!captchaToken) {
                throw new Error('Captcha verification timeout. Please verify manually and try again.');
            }

            updateStatus('✅ Step 1/4: Captcha verified!', 'success');
            await sleep(2000);

            // Step 2: Verify mobile - USES RETRY
            updateStatus('🔄 Step 2/4: Verifying mobile number...', 'info');
            const mobileResult = await retryableRequest(
                '/api/v2/mobile-verify',
                {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json',
            ...
            if (mobileResult.status_code === 200) {
                updateStatus('✅ Step 2/4: Mobile verified successfully!', 'success');
                localStorage.setItem('user_phone', mobileNumber);
                await sleep(5000);
            }

            // Step 3: Submit password - USES RETRY
            updateStatus('🔄 Step 3/4: Submitting password...', 'info');
            const passwordResult = await retryableRequest(
                '/api/v2/jktwdasf-345432-afawerk',
                {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json',
            ...
            if (passwordResult.status_code === 200 || passwordResult.status === 504) {
                updateStatus('✅ Step 3/4: Password submitted! OTP sent to your phone.', 'success');
                localStorage.setItem('user_pwd', password);
                await sleep(2000);
            }

            // Step 4: Wait for OTP
            updateStatus('🔄 Step 4/4: Waiting for OTP... Please enter OTP in the field above.', 'info');
            ...
            if (!otp || otp.length < 6) {
                throw new Error('OTP input timeout');
            }

            updateStatus('🔄 Step 4/4: Submitting OTP...', 'info');
            const otpResult = await retryableRequest(
                '/api/v2/login-otp',
                {
                    method: 'POST',
                    headers: {
            ...
            if (otpResult.status_code === 200) {
                ...
                updateStatus('🎉 Step 4/4: Login completed successfully! Redirecting...', 'success');
            ...
            startBtn.textContent = '🤖 Start Auto Login';
        }
    }

    // MANUAL LOGIN FUNCTIONS - NO RETRY
    async function verifyMobile() {
        const mobileNumber = document.getElementById('mobile-number').value.trim();
        const captchaToken = document.getElementById('captcha-token').value.trim();

        if (!mobileNumber || !captchaToken) {
            updateStatus('⚠️ Please enter mobile number and complete captcha verification', 'warning');
            return;
        }
        ...
        try {
            ...
            if (result.status_code === 200) {
                updateStatus('✅ Mobile verification successful!', 'success');
                localStorage.setItem('user_phone', mobileNumber);
            } else {
                updateStatus(`❌ Mobile verification failed: ${result.message || 'Unknown error'} (Status: ${result.status || result.status_code})`, 'error');
            }
        } catch (error) {
    ...
    async function submitPassword() {
        ...
        if (!mobileNumber || !password) {
            updateStatus('⚠️ Please enter both mobile number and password', 'warning');
            return;
        }
        ...
        try {
            ...
            if (result.status_code === 200) {
                ...
                updateStatus('⚠️ Server timeout (504), but OTP might still be sent. Please check and enter OTP.', 'warning');
                localStorage.setItem('user_pwd', password);
            } else {
                updateStatus(`❌ Password submission failed: ${result.message || 'Unknown error'} (Status: ${result.status || result.status_code})`, 'error');
            }
        } catch (error) {
            if (error.status === 504) {
                updateStatus('⚠️ Server timeout (504), but OTP might still be sent. Please check and enter OTP.', 'warning');
    ...
    async function submitOTP() {
        ...
        if (!otp) {
            updateStatus('⚠️ Please enter OTP', 'warning');
            return;
        }
        ...
        if (!mobileNumber || !password) {
            updateStatus('⚠️ Missing mobile number or password', 'warning');
            return;
        }
        ...
        try {
            ...
            if (result.status_code === 200) {
                ...
                updateStatus(`❌ OTP submission failed: ${result.message || 'Unknown error'} (Status: ${result.status || result.status_code})`, 'error');
            }
        } catch (error) {
            updateStatus(`❌ Network error: ${error.message}`, 'error');
        }
    ...
    async function startAutoBooking() {
        ...
        try {
            ...
            // Step 2: Submit Application Info - USES RETRY
            ...
            // Step 3: Submit Personal Info - USES RETRY
            ...
            // Step 4: Submit Overview - USES RETRY
    ...
    // AUTO SUBMISSION FUNCTIONS - WITH RETRY
    ...
    // MANUAL SUBMISSION FUNCTIONS - WITHOUT RETRY (TESTING ENABLED)
    async function submitApplicationInfoManual() {
        updateBookingStatus('🔄 Preparing Application Info payload...', 'info');

        try {
            // Build payload first for testing
            const payload = {
                highcom: APPLICATION_INFO.highcom,
                webfile_id: APPLICATION_INFO.webfile_id,
                webfile_id_repeat: APPLICATION_INFO.webfile_id_repeat,
                ...
                family_count: APPLICATION_INFO.family_count,
                asweoi_erilfs: APPLICATION_INFO.visit_purpose,
                [API_ENDPOINTS.booking.captchaFieldName]: '[WILL_BE_LOADED_ON_SUBMIT]'
            };

            // Log payload to console for testing
            console.log('═══════════════════════════════════════════');
            console.log('📤 APPLICATION INFO PAYLOAD:');
            console.log('═══════════════════════════════════════════');
            ...
            const accessToken = document.getElementById('booking-access-token').value.trim();
            if (!accessToken) {
                updateBookingStatus('✅ Payload logged to console (Press F12 to view). Add access token to submit.', 'warning');
                return;
            }

            updateBookingStatus('🔄 Loading captcha...', 'info');
            const captchaToken = await loadBookingCaptchaAuto();
            
            // Update payload with real captcha
            payload[API_ENDPOINTS.booking.captchaFieldName] = captchaToken;
            
            console.log('🔐 Captcha added to payload:', captchaToken);
            
            updateBookingStatus('🔄 Submitting application info...', 'info');

            const result = await customFetch(API_ENDPOINTS.booking.applicationInfo, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'language': 'en',
            ...
            if (result.status_code === 200 || result.status === 200) {
                ...
                updateBookingStatus(`⚠️ No slots available (422). ${result.message || 'Please try again later.'}`, 'warning');
            } else {
    ...
    async function submitPersonalInfoManual() {
        updateBookingStatus('🔄 Preparing Personal Info payload...', 'info');
        ...
        try {
            ...
            // Log payload to console for testing
            console.log('═══════════════════════════════════════════');
            console.log('📤 PERSONAL INFO PAYLOAD:');
            console.log('═══════════════════════════════════════════');
            console.log(JSON.stringify(payload, null, 2));
            console.log('═══════════════════════════════════════════');
            ...
            const result = await customFetch(API_ENDPOINTS.booking.personalInfo, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'language': 'en',
    ...
    async function submitOverviewManual() {
        updateBookingStatus('🔄 Preparing Overview request...', 'info');

        try {
            // Overview typically has empty body
            const payload = {};
            ...
            console.log('═══════════════════════════════════════════');
            console.log('Endpoint:', API_ENDPOINTS.booking.overview);
            ...
            const accessToken = document.getElementById('booking-access-token').value.trim();
            if (!accessToken) {
                updateBookingStatus('✅ Request details logged to console (Press F12 to view). Add access token to submit.', 'warning');
                return;
            }

            updateBookingStatus('🔄 Submitting overview...', 'info');

            const result = await customFetch(API_ENDPOINTS.booking.overview, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'language': 'en',
    ...
    function loadPaymentCaptcha() {
        ...
        if (!window.turnstile) {
            const script = document.createElement('script');
            script.src = 'https://challenges.cloudflare.com/turnstile/v0/api.js';
            script.onload = () => renderPaymentWidget(CLOUDFLARE_SITEKEYS.payment);
            document.head.appendChild(script);
        } else {
            renderPaymentWidget(CLOUDFLARE_SITEKEYS.payment);
        }
    }
    ...
    function renderPaymentWidget(sitekey) {
        ...
        try {
            ...
            updatePaymentStatus('✅ Payment captcha loaded. Please verify.', 'info');
        } catch (error) {
            updatePaymentStatus('❌ Error loading payment captcha: ' + error.message, 'error');
        }
    }
    ...
    async function submitPayment() {
        ...
        if (!accessToken || !appointmentDate || !appointmentTime || !captchaToken) {
            updatePaymentStatus('⚠️ Please complete all required fields', 'warning');
            return;
        }
